...
export class MatterAPIImpl {
     ...
     */
    assertMatterReady(cannotAction) {
        const matterManager = this.api._matterManager;
        if (!matterManager) {
            throw new Error(`${cannotAction} before Homebridge has finished launching. Do this from your platform's 'didFinishLaunching' event, not during plugin initialisation.`);
        }
        if (!matterManager.hasActiveMatter()) {
            throw new Error(`${cannotAction}: Matter is not enabled for this bridge. api.matter is available because another bridge uses Matter, but this bridge has no active 'matter' configuration to register against.`);
        }
    }
