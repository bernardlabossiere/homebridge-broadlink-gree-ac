...
__export(Conformance_exports, {
  ...
  collectDotSegments: () => collectDotSegments
});
  ...
  function validateReferences(conformance, ast, errorTarget, resolver) {
    switch (ast.type) {
          ...
          if (referenced?.effectiveMetatype === import_common.Metatype.enum) {
            ...
            operatorResolver = (name) => {
              if (typeof name === "string") {
                const enumValue = enumDef.member(name) ?? enumDef.member((0, import_general.camelize)(name, true));
                if (enumValue) {
                  return enumValue;
                }
              }
      ...
      case "." /* DOT */: {
        ...
        if (segments !== void 0 && !resolver(segments)) {
          errorTarget.error(
...
function ParsedAst(conformance, definition) {
  ...
  function isChoiceIndicator(value) {
  ...
  function parseChoice(expr) {
    ...
    if (tokens.token?.type !== "word") {
      ...
      let dotExpr = {
        type: "." /* DOT */,
        param: { lhs: expr, rhs: { type: "name" /* Name */, param: tokens.token.value } }
      };
      tokens.next();
      while (atOperator(".") && tokens.peeked?.type === "word" && !isChoiceIndicator(tokens.peeked.value)) {
        ...
        dotExpr = {
          type: "." /* DOT */,
          param: { lhs: dotExpr, rhs: { type: "name" /* Name */, param: tokens.token.value } }
        };
        tokens.next();
      }
  ...
  function parseAtomicExpressionWithoutChoice() {
    ...
    if (tokens.token.type === "word") {
      ...
      switch (name.toLowerCase()) {
        ...
        case "true":
          return { type: "value" /* Value */, param: true };
        case "false":
          return { type: "value" /* Value */, param: false };
      }
      if (name === "Rev" && atOperator(">=")) {
        ...
        if (tokens.token?.type === "word") {
          ...
          if (versionMatch) {
            tokens.next();
            return { type: "revision" /* Revision */, param: Number.parseInt(versionMatch[1]) };
          }
        }
      }
...
function collectDotSegments(ast) {
  if (ast.type === "name" /* Name */) {
    return [ast.param];
  }
  if (ast.type === "." /* DOT */) {
