...
const cloneEntry = (val) => Array.isArray(val) ? [...val] : isObject(val) ? { ...val } : val;
      ...
      return {
        ...
        next() {
          while (true) {
            ...
            if (!next.done && isObject(next.value) && next.value.fabricIndex !== fabric) {
              continue;
            }
            return next;
          }
...
function createProxy(config, reference, session) {
  const isFabricFiltered = config.manageEntries && config.fabricScoped && hasRemoteActor(session) && (session.fabricFiltered || config.fabricSensitive);
