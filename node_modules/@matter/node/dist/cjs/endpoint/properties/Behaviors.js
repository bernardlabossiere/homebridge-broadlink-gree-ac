...
class Behaviors {
      ...
      if (schema instanceof import_model.ClusterModel) {
        ...
        if (features.size) {
          ...
          elementDiagnostic.push([
            import_general.Diagnostic.strong("features"),
            [...features].map((name2) => titleMap.get(name2) ?? name2)
          ]);
        }
      }
  ...
  #augmentEndpoint(type) {
    ...
    const get = () => {
      ...
      if (status === import_general.Lifecycle.Status.Destroying || status === import_general.Lifecycle.Status.Destroyed) {
        return void 0;
      }
