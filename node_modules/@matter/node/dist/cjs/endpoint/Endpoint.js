...
class Endpoint {
  ...
  stateOf(type) {
    return this.#requirePresent(type, this.maybeStateOf(type));
  }
  maybeStateOf(type) {
    ...
    await this.act(`set<${this}>`, async (agent) => {
      ...
      for (const behaviorId in values) {
        ...
        if (!(behavior instanceof import_Behavior.Behavior)) {
          throw new import_errors.EndpointBehaviorNotPresentError(behaviorId);
        }
  ...
  async setStateOf(type, values) {
    if (typeof type === "string") {
      ...
      if (type === void 0) {
        throw new import_errors.EndpointBehaviorNotPresentError(typeName);
      }
    }
  ...
  async getStateOf(type, selector, options) {
    const id = typeof type === "string" ? type : type.id;
    if (!this.behaviors.has(type)) {
      throw new import_errors.EndpointBehaviorNotPresentError(id);
    }
  ...
  #installedClusterBehavior(type) {
    if (typeof type === "string") {
      const installed = this.behaviors.supported[type];
      return installed !== void 0 && import_ClusterBehavior.ClusterBehavior.isType(installed) ? installed : void 0;
    }
    return this.behaviors.has(type) ? this.behaviors.typeFor(type) : void 0;
  }
  /**
   * Resolve an installed cluster behavior, throwing if the behavior is absent or not a cluster behavior.
   */
  #requireClusterBehavior(type) {
    const id = typeof type === "string" ? type : type.id;
    if (typeof type === "string") {
      ...
      if (!import_ClusterBehavior.ClusterBehavior.isType(installed)) {
        throw new import_errors.EndpointBehaviorNotClusterError(id);
      }
      return installed;
    }
    if (!this.behaviors.has(type)) {
  ...
  #requirePresent(type, result) {
    if (result === void 0) {
      throw new import_errors.EndpointBehaviorNotPresentError(typeof type === "string" ? type : type.id);
    }
    return result;
  }
  get construction() {
  ...
  #resolveClientPeerContext() {
    const root = this.ownerOfType(import_root.RootEndpoint);
    if (root === void 0 || root.nodeType !== "client") {
      return void 0;
    }
    const commissioning = root.behaviors.maybeStateOf("commissioning");
    if (commissioning?.fabricIndexOnPeer !== void 0) {
      return { fabricIndexOnPeer: commissioning.fabricIndexOnPeer };
    }
    const opcreds = root.behaviors.maybeStateOf("operationalCredentials");
    const fallback = opcreds?.currentFabricIndex;
    if (fallback !== void 0 && fallback !== import_types.FabricIndex.NO_FABRIC) {
  ...
  #resolveSelection(selector) {
    ...
    for (const id of Object.keys(selector)) {
      ...
      const raw = selector[id];
      if (raw === void 0) continue;
      map.set(id, raw);
    }
    return map;
  }
  async #performRead(selector, options) {
    const selection = this.#resolveSelection(selector);
    const attributePaths = new import_protocol.Read.AttributePaths();
    const clusterLookup = /* @__PURE__ */ new Map();
    for (const [id, raw] of selection) {
      ...
      const clusterId = schema.id;
      if (clusterId === void 0) {
        continue;
      }
      const attrs = /* @__PURE__ */ new Map();
      const attrNameToId = /* @__PURE__ */ new Map();
      for (const attr of schema.attributes) {
        if (attr.id !== void 0 && attr.propertyName !== void 0) {
      ...
      for (const name of raw) {
        if (!allowedNames.has(name)) {
          throw new import_errors.AttributeNotPresentError(id, name);
        }
    ...
    const nodeEndpoint = this.ownerOfType(import_root.RootEndpoint);
    if (nodeEndpoint === void 0) {
      throw new import_general.ImplementationError(`${this} is not installed in a node`);
    }
    const node = nodeEndpoint;
    const failed = new Array();
    if (node.nodeType === "group") {
      throw new import_errors.InvalidGroupOperationError("Groups do not support reading attributes");
    }
    if (node.nodeType === "client") {
      const fabricFilter = options?.fabricFilter ?? true;
      const baseRequest = (0, import_protocol.Read)({ attributes: [...attributePaths.paths], fabricFilter });
      const request2 = options?.includeKnownVersions ? { ...baseRequest, includeKnownVersions: true } : baseRequest;
      const readValues = /* @__PURE__ */ new Map();
      for await (const chunk of node.interaction.read(request2, void 0)) {
        for await (const report of chunk) {
          ...
          } else if (report.kind === "attr-status" && report.status !== import_types.Status.Success) {
            failed.push({ path: report.path, status: report.status, clusterStatus: report.clusterStatus });
          }
        }
      }
      const stateSlice = this.#assembleSlice(selection);
      const partial2 = {};
      for (const [id] of selection) {
        const stateValues = stateSlice[id] ?? {};
        const freshValues = readValues.get(id);
        partial2[id] = freshValues?.size ? { ...stateValues, ...Object.fromEntries(freshValues) } : stateValues;
      }
      if (failed.length > 0) {
        this.#excludeFailedPaths(partial2, failed, clusterLookup);
        throw new import_errors.EndpointReadFailedError({ failed, partial: partial2 });
      }
      return partial2;
    }
    const collectFailures = async (stream) => {
      for await (const chunk of stream) {
        for await (const report of chunk) {
          if (report.kind === "attr-status" && report.status !== import_types.Status.Success) {
            failed.push({ path: report.path, status: report.status, clusterStatus: report.clusterStatus });
          }
        }
      }
    };
    const request = (0, import_protocol.Read)({ attributes: [...attributePaths.paths], fabricFilter: false });
    await import_LocalActorContext.LocalActorContext.act(
      "endpoint-get",
      (context) => collectFailures(node.interaction.read(request, context))
    );
    const partial = this.#assembleSlice(selection);
    if (failed.length > 0) {
      this.#excludeFailedPaths(partial, failed, clusterLookup);
      throw new import_errors.EndpointReadFailedError({ failed, partial });
    }
    return partial;
  }
  #excludeFailedPaths(partial, failed, clusterLookup) {
