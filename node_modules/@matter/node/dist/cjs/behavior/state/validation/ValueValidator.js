...
var import_FabricIndexSentinel = require("./FabricIndexSentinel.js");
/**
 * @license
 * Copyright 2022-2026 Matter.js Authors
 * SPDX-License-Identifier: Apache-2.0
 */
const logger = import_general.Logger.get("ValueValidator");
  ...
  switch (metabase?.metatype) {
    ...
    case void 0:
      ...
      if (type === void 0) {
        ...
        throw new import_protocol.SchemaImplementationError(new import_model.DataModelPath(schema.path), `No type defined`);
      }
      throw new import_protocol.SchemaImplementationError(
        new import_model.DataModelPath(schema.path),
...
function createIntegerValidator(schema, supervisor, name) {
  ...
  if ((0, import_FabricIndexSentinel.isFabricIndexSentinel)(schema)) {
    const fabricSentinelOrSubstitute = (value, session, location) => {
      const peerContext = session.clientPeerContext;
      if (value !== import_types.FabricIndex.OMIT_FABRIC || peerContext === void 0) {
        inner(value, session, location);
        return;
      }
      const siblings = location.siblings;
      if (siblings !== void 0 && peerContext.fabricIndexOnPeer !== void 0) {
...
function createStructValidator(schema, supervisor) {
  ...
  const validateStruct = (struct, session, location) => {
    ...
    const config = location.config ?? (0, import_SupervisionConfig.maybeConfigOf)(struct);
    const sublocation = {
