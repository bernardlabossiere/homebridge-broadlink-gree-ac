...
const localInteractionBeginEmitted = /* @__PURE__ */ new WeakSet();
...
class RootReference {
  ...
  #startWrite() {
    ...
    if (interactionComplete && !interactionComplete.isObservedBy(this.#internals.interactionObserver)) {
      let emitBegin;
      if ((0, import_protocol.hasRemoteActor)(this.#session)) {
      ...
      if (emitBegin && this.#internals.events?.interactionBegin?.isObserved) {
        let handleBeginObserverError2 = function(error) {
          logger.warn(`Error in ${location.path} observer:`, error);
        };
        var handleBeginObserverError = handleBeginObserverError2;
        const location = this.#internals.location;
        try {
          const result = this.#internals.events?.interactionBegin?.emit(this.#session);
          if (import_general.MaybePromise.is(result)) {
            import_general.MaybePromise.then(result, void 0, handleBeginObserverError2);
          }
        } catch (e) {
          handleBeginObserverError2(e);
        }
      }
