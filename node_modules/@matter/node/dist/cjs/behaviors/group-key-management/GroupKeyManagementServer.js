...
var import_group_key_management = require("@matter/types/clusters/group-key-management");
...
const groupKeySetStruct = import_GroupKeyManagementBehavior.GroupKeyManagementBehavior.schema.datatypes.require("GroupKeySetStruct");
...
class GroupKeyManagementServer extends import_GroupKeyManagementBehavior.GroupKeyManagementBehavior {
  ...
  #validateGroupKeyMap(groupKeyMap) {
    ...
    for (const entry of groupKeyMap) {
      ...
      if (knownGroupIds.has(id)) {
        throw new import_types.StatusResponseError(
          ...
          import_types.Status.ConstraintError
        );
      }
    ...
    if (groupIdsPerFabric.some((count) => count > this.state.maxGroupsPerFabric)) {
      throw new import_types.StatusResponseError(
        ...
        import_types.Status.ResourceExhausted
      );
    }
  }
  ...
  async keySetWrite({ groupKeySet }) {
    ...
    if (epochKey0 === null || epochStartTime0 === null) {
      throw new import_types.StatusResponseError("EpochKey0 and EpochStartTime0 must be set", import_types.Status.InvalidCommand);
    }
    if (epochStartTime0 <= import_protocol.IPK_DEFAULT_EPOCH_START_TIME) {
      throw new import_types.StatusResponseError("EpochStartTime0 must not be 0", import_types.Status.InvalidCommand);
    }
    ...
    if (epochKey1 === null && epochStartTime1 !== null) {
      throw new import_types.StatusResponseError("EpochKey1 must be set if EpochStartTime1 is set", import_types.Status.InvalidCommand);
    }
    if (epochKey2 !== null && epochKey1 === null) {
      throw new import_types.StatusResponseError("EpochKey1 must be set if EpochKey2 is set", import_types.Status.InvalidCommand);
    }
    ...
    if (epochKey2 === null && epochStartTime2 !== null) {
      throw new import_types.StatusResponseError("EpochKey2 must be set if EpochStartTime2 is set", import_types.Status.InvalidCommand);
    }
    if (groupKeySecurityPolicy !== import_group_key_management.GroupKeyManagement.GroupKeySecurityPolicy.TrustFirst) {
      throw new import_types.StatusResponseError("GroupKeySecurityPolicy must be TrustFirst", import_types.Status.InvalidCommand);
    }
    if (groupKeyMulticastPolicy !== import_group_key_management.GroupKeyManagement.GroupKeyMulticastPolicy.PerGroupId) {
      throw new import_types.StatusResponseError("GroupKeyMulticastPolicy must be PerGroupId", import_types.Status.InvalidCommand);
    }
  ...
  keySetRead({
    ...
    if (groupKeySet === void 0) {
      throw new import_types.StatusResponseError(`GroupKeySet ${groupKeySetId} not found`, import_types.Status.NotFound);
    }
    return {
      groupKeySet: {
        ...groupKeySet,
  ...
  async keySetRemove({ groupKeySetId }) {
    if (groupKeySetId === 0) {
      throw new import_types.StatusResponseError(`GroupKeySet ${groupKeySetId} cannot be removed`, import_types.Status.InvalidCommand);
    }
    (0, import_protocol.assertRemoteActor)(this.context);
    const fabric = this.context.session.associatedFabric;
    const fabricIndex = fabric.fabricIndex;
