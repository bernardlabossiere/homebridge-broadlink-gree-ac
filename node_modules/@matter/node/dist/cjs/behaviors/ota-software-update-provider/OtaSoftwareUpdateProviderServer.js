...
var import_access_control2 = require("@matter/types/clusters/access-control");
var import_ota_software_update_provider = require("@matter/types/clusters/ota-software-update-provider");
...
class OtaSoftwareUpdateProviderServer extends import_OtaSoftwareUpdateProviderBehavior.OtaSoftwareUpdateProviderBehavior {
      ...
      if (!node.stateOf(import_access_control.AccessControlServer).acl.some(
        ...
        const acl = [
          ...node.stateOf(import_access_control.AccessControlServer).acl,
          {
            ...
            subjects: [],
            targets: [{ endpoint: this.endpoint.number, cluster: import_ota_software_update_provider.OtaSoftwareUpdateProvider.id }]
          }
        ];
  ...
  async queryImage(request) {
    ...
    if (updateInProgress !== void 0) {
      ...
      if (sessionFromBdxSession === void 0) {
        this.#removeInProgressDetails(peerAddress);
      } else if (sessionFromBdxSession.id !== session.id && sessionFromBdxSession.activeTimestamp < session.activeTimestamp) {
        try {
          ...
          return {
            ...
            delayedActionTime: import_general.Seconds.of((0, import_general.Minutes)(2))
          };
        }
        await this.endpoint.act(
          (agent) => agent.get(import_SoftwareUpdateManager.SoftwareUpdateManager).suppressNextStartUp(peerAddress, session.id)
        );
      } else {
        logger.info(
        ...
        return {
          status: import_ota_software_update_provider.OtaSoftwareUpdateProvider.Status.Busy,
          delayedActionTime: import_general.Seconds.of((0, import_general.Minutes)(2))
          // 2 minutes is enough since reboot-triggered retries are handled above
        };
      }
    }
