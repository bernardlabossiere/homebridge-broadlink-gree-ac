...
var import_window_covering = require("@matter/types/clusters/window-covering");
...
class WindowCoveringBaseServer extends WindowCoveringBase {
  ...
  #handleModeChanging(mode) {
    ...
    if (mode.maintenanceMode && !this.state.supportsMaintenanceMode) {
      throw new import_types.StatusResponseError("Maintenance mode not supported", import_types.Status.ConstraintError);
    }
    this.internal.inMaintenanceMode = !!mode.maintenanceMode;
    if (mode.calibrationMode) {
      if (!this.internal.supportsCalibration) {
        throw new import_types.StatusResponseError("Calibration not supported", import_types.Status.ConstraintError);
      }
  ...
  #assertMotionLockStatus() {
    if (this.internal.inMaintenanceMode) {
      throw new import_types.StatusResponseError("Device is in maintenance mode", import_types.Status.Busy);
    }
    switch (this.internal.calibrationMode) {
      case 0 /* Enabled */:
        if (!this.internal.supportsCalibration) {
          throw new import_types.StatusResponseError("Calibration not implemented", import_types.Status.Failure);
        }
        break;
      case 1 /* Running */:
        break;
    ...
    if (!this.state.configStatus.operational) {
      throw new import_types.StatusResponseError("Device is not operational", import_types.Status.Failure);
    }
  }
  /**
...
class WindowCoveringServer extends WindowCoveringBaseServer.for(import_window_covering.WindowCovering) {
}
