...
const { commands } = import_operational_credentials.OperationalCredentials.schema;
const attestationRequest = commands.require("AttestationRequest");
const csrRequest = commands.require("CsrRequest");
const OperationalCredentialsSchema = import_operational_credentials.OperationalCredentials.schema.extend(
  void 0,
  attestationRequest.extend(void 0, attestationRequest.fields.extend("AttestationNonce", { constraint: "none" })),
  csrRequest.extend(void 0, csrRequest.fields.extend("CsrNonce", { constraint: "none" }))
);
const OperationalCredentialsBase = import_OperationalCredentialsBehavior.OperationalCredentialsBehavior.for(
  import_operational_credentials.OperationalCredentials,
  OperationalCredentialsSchema
);
class OperationalCredentialsServer extends OperationalCredentialsBase {
  initialize() {
  ...
  async attestationRequest({ attestationNonce }) {
    ...
    if (attestationNonce.byteLength !== 32) {
      throw new import_types.StatusResponseError("Invalid attestation nonce length", import_types.Status.InvalidCommand);
    }
  ...
  async csrRequest({ csrNonce, isForUpdateNoc }) {
    ...
    if (csrNonce.byteLength !== 32) {
      throw new import_types.StatusResponseError("Invalid csr nonce length", import_types.Status.InvalidCommand);
    }
    const session = this.context.session;
    import_protocol.NodeSession.assert(session);
    if (isForUpdateNoc && session.isPase) {
      throw new import_types.StatusResponseError("csrRequest for UpdateNoc received on a PASE session", import_types.Status.InvalidCommand);
    }
  ...
  async addNoc({
    ...
    try {
      ...
      await this.endpoint.act(
        (agent) => agent.get(import_access_control.AccessControlServer).addDefaultCaseAcl(fabric, [caseAdminSubject])
      );
    } catch (e) {
    ...
    logger.info(
      `addNoc success, adminVendorId ${adminVendorId}, caseAdminSubject ${import_types.SubjectId.strOf(caseAdminSubject)}`
    );
    return {
  ...
  async addTrustedRootCertificate({
    ...
    try {
      ...
      if (error instanceof import_general.CryptoVerifyError || error instanceof import_general.CertificateError || error instanceof import_types.ValidationError || error instanceof import_general.UnexpectedDataError) {
        throw new import_types.StatusResponseError(error.message, import_types.Status.InvalidCommand);
      }
      throw error;
    }
    const fabrics = this.env.get(import_protocol.FabricManager);
