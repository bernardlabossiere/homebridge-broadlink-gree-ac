...
var import_color_control = require("@matter/types/clusters/color-control");
...
class ColorControlBaseServer extends ColorControlBase {
  ...
  #assertStepSize(stepSize, errorContext) {
    if (stepSize === 0) {
      throw new import_types.StatusResponseError(`${errorContext} step size must not be 0`, import_types.Status.InvalidCommand);
    }
  }
  /**
   * Default implementation notes:
  ...
  stepColor({
    ...
    if (stepX === 0 && stepY === 0) {
      throw new import_types.StatusResponseError("Color step sizes must not be 0", import_types.Status.InvalidCommand);
    }
    if (!this.#optionsAllowExecution(optionsMask, optionsOverride)) {
      return;
    }
  ...
  #assertRate(mode, rate) {
    if ((mode === import_color_control.ColorControl.MoveMode.Up || mode === import_color_control.ColorControl.MoveMode.Down) && rate === 0) {
      throw new import_types.StatusResponseError("Rate must not be 0 when moving Up or Down", import_types.Status.InvalidCommand);
    }
  }
...
class ColorControlServer extends ColorControlBaseServer.for(import_color_control.ColorControl) {
}
