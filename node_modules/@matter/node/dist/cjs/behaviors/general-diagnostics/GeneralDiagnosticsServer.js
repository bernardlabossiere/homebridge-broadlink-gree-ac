...
var import_general_diagnostics = require("@matter/types/clusters/general-diagnostics");
...
class GeneralDiagnosticsServer extends Base {
  ...
  #validateTestEnabledKey(enableKey) {
    ...
    if (keyData.every((byte) => byte === 0)) {
      throw new import_types.StatusResponseError("Invalid test enable key, all zeros", import_types.Status.ConstraintError);
    }
    ...
    keyData.forEach((byte, index) => {
      if (byte !== expectedKeyData[index]) {
        throw new import_types.StatusResponseError("Invalid test enable key", import_types.Status.ConstraintError);
      }
    });
  }
  ...
  triggerTestEvent(eventTrigger) {
    throw new import_types.StatusResponseError(`Unsupported test event trigger ${eventTrigger}`, import_types.Status.InvalidCommand);
  }
  timeSnapshot() {
  ...
  payloadTestRequest({
    ...
    if (!this.state.testEventTriggersEnabled) {
      throw new import_types.StatusResponseError("Test event triggers are disabled", import_types.Status.ConstraintError);
    }
      ...
      invokeResponses: [
        {
          command: {
            commandPath: {
              ...
              clusterId: import_general_diagnostics.GeneralDiagnostics.id,
              commandId: (0, import_types.CommandId)(4)
              // Hardcode for now
            },
            commandRef: 0,
            commandFields: (0, import_types.TlvOfModel)(
              import_general_diagnostics.GeneralDiagnostics.schema.commands.require("PayloadTestResponse")
            ).encodeTlv({
              payload
            })
          }
    ...
    if (responseSize > exchange.maxPayloadSize) {
      throw new import_types.StatusResponseError("Response too large", import_types.Status.ResourceExhausted);
    }
    return {
      payload
    };
