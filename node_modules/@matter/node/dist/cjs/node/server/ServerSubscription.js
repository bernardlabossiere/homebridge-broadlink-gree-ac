...
class ServerSubscription {
  ...
  async handlePeerCancel() {
    ...
    try {
      await this.#currentSendExchange?.close(new import_general.ClosedError("Subscription cancelled by peer"));
    } catch (error) {
  ...
  async #sendUpdateMessage(lifetime, attributeFilter, eventsMinNumber, onlyWithData, session) {
    ...
    try {
      ...
      var _stack3 = [];
      try {
        if (import_types.StatusResponseError.is(error, import_types.Status.InvalidSubscription, import_types.Status.Failure)) {
      ...
      var _stack4 = [];
      try {
        this.#currentSendExchange = void 0;
