...
const DESCRIPTOR_ID = import_descriptor.Descriptor.id;
const DEVICE_TYPE_LIST_ATTR_ID = import_descriptor.Descriptor.attributes.deviceTypeList.id;
const SERVER_LIST_ATTR_ID = import_descriptor.Descriptor.attributes.serverList.id;
const PARTS_LIST_ATTR_ID = import_descriptor.Descriptor.attributes.partsList.id;
...
function getStoreValue(values, id, name) {
  if (values === void 0) {
    return void 0;
  }
...
class ClientStructure {
   ...
   */
  eventEmitter;
  constructor(node, storeFactory, options) {
    this.#node = node;
   ...
   */
  async applyWireChanges(changes) {
    for (const change of changes) {
      switch (change.kind) {
        case "update": {
          const endpointNumber = change.endpoint;
          const endpoint = this.#endpointFor(endpointNumber);
          const cluster = this.#clusterForBehavior(endpoint, change.behavior);
          const values = new Map(Object.entries(change.changes));
          if (typeof change.version === "number") {
            values.set(import_DatasourceCache.DatasourceCache.VERSION_KEY, change.version);
          }
  ...
  get subscribedFabricFiltered() {
    if (this.#subscribedFabricFiltered === void 0) {
      ...
      try {
        ...
        const defaultSubscription = network?.defaultSubscription;
        ...
        const events = this.#node.events;
        const networkEvents = events?.network;
        const changedEvent = networkEvents?.defaultSubscription$Changed;
        changedEvent?.on((newSubscription) => {
  ...
  async #mutateAttribute(change, scope, currentUpdates) {
    ...
    if (currentUpdates === void 0) {
      currentUpdates = {
        endpointId,
        clusterId,
        values: /* @__PURE__ */ new Map([[attributeId, change.value]])
      };
      if (scope.isWildcard(endpointId, clusterId)) {
        currentUpdates.values.set(import_DatasourceCache.DatasourceCache.VERSION_KEY, change.version);
      }
    } else {
  ...
  async #updateCluster(attrs) {
    ...
    if (cluster.behavior && attrs.values.has(import_model.FeatureMap.id)) {
      if (!(0, import_general.isDeepEqual)(cluster.features, attrs.values.get(import_model.FeatureMap.id))) {
        cluster.behavior = void 0;
      }
    }
    if (cluster.behavior && attrs.values.has(import_model.AttributeList.id)) {
      const attributeList = attrs.values.get(import_model.AttributeList.id);
    ...
    if (cluster.behavior && attrs.values.has(import_model.AcceptedCommandList.id)) {
      const acceptedCommands = attrs.values.get(import_model.AcceptedCommandList.id);
  ...
  #synchronizeCluster(structure, cluster) {
    ...
    if (cluster.behavior === void 0) {
      if (cluster.store.initialValues) {
        ...
        const clusterRevision = getStoreValue(values, import_model.ClusterRevision.id, "clusterRevision");
        const features = getStoreValue(values, import_model.FeatureMap.id, "featureMap");
        const attributeList = getStoreValue(values, import_model.AttributeList.id, "attributeList");
        const commandList = getStoreValue(values, import_model.AcceptedCommandList.id, "acceptedCommandList");
        const generatedCommandList = getStoreValue(values, import_model.GeneratedCommandList.id, "generatedCommandList");
      ...
      if (
        ...
        const behaviorType = (0, import_PeerBehavior.PeerBehavior)(shape);
        if (endpoint.lifecycle.isInstalled) {
          cluster.pendingBehavior = behaviorType;
          if (cluster.pendingDelete) {
            logger.warn(
              `Cluster 0x${import_general.hex.fixed(cluster.id, 8)} on ${endpoint} is absent from descriptor server list but peer sent attribute data for it; keeping cluster`
            );
    ...
    if (cluster.id === import_descriptor.Descriptor.id) {
      let attrs;
  ...
  #synchronizeDescriptor(structure, attrs) {
    const { endpoint } = structure;
    const deviceTypeList = getStoreValue(attrs, DEVICE_TYPE_LIST_ATTR_ID, DEVICE_TYPE_LIST_ATTR_NAME);
    if (Array.isArray(deviceTypeList)) {
      ...
      for (const dt of deviceTypeList) {
        ...
        let isApp = false;
        const model = import_model.Matter.deviceTypes(dt.deviceType);
        if (model !== void 0) {
   ...
   */
  #clusterForBehavior(endpoint, behaviorId) {
    const supported = endpoint.endpoint.behaviors.supported[behaviorId];
    if (supported?.cluster) {
      return this.#clusterFor(endpoint, supported.cluster.id);
    }
    for (const cluster2 of endpoint.clusters.values()) {
      if (cluster2.behavior?.id === behaviorId) {
        return cluster2;
      }
    }
    const numericId = Number.parseInt(behaviorId);
    if (Number.isFinite(numericId)) {
      return this.#clusterFor(endpoint, numericId);
    }
    const clusterModel = import_model.Matter.clusters((0, import_general.capitalize)(behaviorId));
    if (clusterModel) {
      return this.#clusterFor(endpoint, clusterModel.id);
    }
  ...
  async #erase(structure) {
    ...
    if (endpoint.construction.status === import_general.Lifecycle.Status.Destroyed) {
      return;
    }
    try {
  ...
  async #rebuild(structure) {
    ...
    for (const cluster of clusters.values()) {
      ...
      if (pendingDelete) {
        ...
        try {
          await import_general.MaybePromise.then(
            cluster.store.erase?.()
          );
        } catch (e) {
