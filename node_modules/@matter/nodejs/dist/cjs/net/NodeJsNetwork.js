...
var import_NodeJsTcpListener = require("./NodeJsTcpListener.js");
var import_NodeJsUdpSocket = require("./NodeJsUdpSocket.js");
/**
 * @license
...
class NodeJsNetwork extends import_general.Network {
  ...
  createUdpSocket(options) {
    return import_NodeJsUdpSocket.NodeJsUdpSocket.create(options);
  }
  createTcpListener(options) {
    return import_NodeJsTcpListener.NodeJsTcpListener.create(options);
  }
  async connectTcp(host, port, options) {
    if (options?.abort?.aborted) {
      throw new import_general.NetworkError("TCP connect aborted");
    }
    return new Promise((resolve, reject) => {
      ...
      const socket = (0, import_node_net.createConnection)({ host, port, noDelay: true }, () => {
        socket.setTimeout(0);
        socket.off("error", rejectOnce);
        settle(() => resolve(new import_NodeJsTcpConnection.NodeJsTcpConnection(socket)));
      });
      const rejectOnce = (error) => settle(() => {
        socket.destroy();
        reject(error);
      });
      abortListener = () => rejectOnce(new import_general.NetworkError("TCP connect aborted"));
      socket.setTimeout(options?.timeout ?? import_general.TCP_CONNECTION_TIMEOUT_MS);
      socket.once("timeout", () => {
        socket.destroy();
        settle(() => reject(new import_general.NetworkError("TCP connection timeout")));
      });
