...
function parameterSignatureOf(parameters) {
  return [...parameters.keys()].sort().map((key) => `${key}=${Bytes.toHex(parameters.raw(key))}`).join("\0");
}
...
function ipOf(record) {
  ...
  if (record.recordType === DnsRecordType.AAAA) {
    if (record.sourceIntf !== void 0 && ServerAddress.isIpv6LinkLocal(record.value) && !record.value.includes("%")) {
