...
class IpService {
  ...
  #onServiceChanged = async ({ updated, deleted }) => {
    ...
    if (updated) {
      for (const record of updated) {
        ...
        if (service) {
          this.#updateService(record.ttl, service);
        } else if (record.recordType === import_DnsCodec.DnsRecordType.TXT) {
          txtUpdated = true;
        }
      }
    }
  ...
  #updateAddress(service, ip) {
    const address = { ip, port: service.port };
...
function parameterSignatureOf(parameters) {
  return [...parameters.keys()].sort().map((key) => `${key}=${import_Bytes.Bytes.toHex(parameters.raw(key))}`).join("\0");
}
function hostKeyOf(name, port) {
  return `${name}:${port}`;
}
function ipOf(record) {
  if (record.recordType === import_DnsCodec.DnsRecordType.A) {
    return record.value;
  }
  if (record.recordType === import_DnsCodec.DnsRecordType.AAAA) {
    if (record.sourceIntf !== void 0 && import_ServerAddress.ServerAddress.isIpv6LinkLocal(record.value) && !record.value.includes("%")) {
