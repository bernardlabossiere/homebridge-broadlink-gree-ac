...
class ControllerCommissioningFlow {
  ...
  /** Helper method to check for errorCode/debugTest responses and throw error on failure */
    ...
    logger.debug(
      `Commissioning step ${context} returned ${OperationalCredentials.NodeOperationalCertStatus[statusCode]} (${statusCode})${debugText ? `, ${debugText}` : ""}${fabricIndex !== void 0 ? `, fabricIndex: ${fabricIndex}` : ""}`
    );
    ...
    throw new CommissioningError(
      `Commission error for "${context}": ${OperationalCredentials.NodeOperationalCertStatus[statusCode]} (${statusCode})${debugText ? `, ${debugText}` : ""}${fabricIndex !== void 0 ? `, fabricIndex: ${fabricIndex}` : ""}`
    );
  }
  /** Helper method to check for errorCode/debugTest responses and throw error on failure */
  ...
  async #configureNetworkThread() {
    ...
    if (!this.commissioningOptions.threadNetwork?.networkName) {
      ...
      if (networkingStatus2 !== NetworkCommissioning.NetworkCommissioningStatus.Success) {
        threadScanFailureHint = `scan failed: status=${NetworkCommissioning.NetworkCommissioningStatus[networkingStatus2]} (${networkingStatus2})${debugText2 ? `, ${debugText2}` : ""}`;
        logger.warn(
          `Thread network scan returned status ${NetworkCommissioning.NetworkCommissioningStatus[networkingStatus2]} (${networkingStatus2})${debugText2 ? `: ${debugText2}` : ""} - attempting connection anyway`
        );
    ...
    if (addNetworkingStatus !== NetworkCommissioning.NetworkCommissioningStatus.Success) {
      throw new ThreadNetworkSetupFailedError(
        `Commissionee failed to add Thread network: status=${NetworkCommissioning.NetworkCommissioningStatus[addNetworkingStatus]} (${addNetworkingStatus})${addDebugText ? `, ${addDebugText}` : ""}${threadScanFailureHint !== void 0 ? ` (${threadScanFailureHint} - verify network name and availability)` : ""}`
      );
    }
    if (networkIndex === void 0) {
    ...
    if (networkingStatus !== NetworkCommissioning.NetworkCommissioningStatus.Success) {
      throw new ThreadNetworkSetupFailedError(
        `Commissionee failed to connect to Thread network: status=${NetworkCommissioning.NetworkCommissioningStatus[networkingStatus]} (${networkingStatus})${debugText ? `, ${debugText}` : ""}${threadScanFailureHint !== void 0 ? ` (${threadScanFailureHint} - verify network name and availability)` : ""}`
      );
    }
    logger.debug(
