...
class InteractionMessenger {
  ...
  trackEndMessage(promise, errorMessage) {
    ...
    this.#endMessagePromise = promise.catch((error) => logger.info(this.exchange.via, errorMessage, Diagnostic.errorMessage(error))).finally(() => this.#endMessagePromise = void 0);
  }
...
class InteractionServerMessenger extends InteractionMessenger {
  ...
  async sendDataReportMessage(dataReport, waitForAck = true) {
    ...
    if (dataReportToSend.suppressResponse) {
      ...
      await this.waitForSuccess("DataReport", { timeout: waitForAck ? void 0 : Millis(500) });
    }
  }
  /**
  ...
  async sendWriteResponse(response, options) {
    await this.send(7 /* WriteResponse */, TlvWriteResponse.encode(response), {
  ...
  async sendInvokeResponse(response) {
    await this.send(9 /* InvokeResponse */, TlvInvokeResponseForSend.encode(response));
  }
}
class IncomingInteractionClientMessenger extends InteractionMessenger {
  ...
  #logContextOf(report) {
    return {
      subId: report.subscriptionId === void 0 ? void 0 : Subscription.idStrOf(report.subscriptionId),
...
class InteractionClientMessenger extends IncomingInteractionClientMessenger {
  ...
  async sendReadRequest(readRequest, options) {
    await this.send(2 /* ReadRequest */, this.#encodeReadingRequest(TlvReadRequest, readRequest), options);
  }
  ...
  async sendInvokeCommand(invokeRequest, options) {
    ...
    await this.send(8 /* InvokeRequest */, TlvInvokeRequest.encode(invokeRequest), options);
  ...
  async sendWriteCommand(writeRequest, options) {
    if (writeRequest.suppressResponse) {
      await this.requestWithSuppressedResponse(6 /* WriteRequest */, TlvWriteRequest, writeRequest, options);
    } else {
