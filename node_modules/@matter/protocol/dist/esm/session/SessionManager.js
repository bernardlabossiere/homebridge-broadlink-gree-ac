...
import { GroupId, NodeId } from "@matter/types";
...
class ShutdownError extends ClosedError {
...
class SessionManager {
  ...
  forFabric(fabric) {
    ...
    return [...this.#sessions].filter(
      (session) => NodeSession.is(session) && session.isSecure && session.fabric?.fabricIndex === fabric.fabricIndex
    );
  }
  sessionFor(peer) {
  ...
  findResumptionRecordById(resumptionId) {
    ...
    const record = [...this.#resumptionRecords.values()].find(
      (record2) => Bytes.areEqual(record2.resumptionId, resumptionId)
    );
    if (record !== void 0) {
