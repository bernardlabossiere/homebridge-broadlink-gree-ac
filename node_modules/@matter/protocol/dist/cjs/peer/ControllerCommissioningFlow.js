...
var import_DeviceAttestationValidator = require("../certificate/DeviceAttestationValidator.js");
...
const CASE_RECONNECT_TIMEOUT = (0, import_general.Minutes)(5);
...
class ControllerCommissioningFlow {
  ...
  /** Helper method to check for errorCode/debugTest responses and throw error on failure */
    ...
    logger.debug(
      `Commissioning step ${context} returned ${import_operational_credentials.OperationalCredentials.NodeOperationalCertStatus[statusCode]} (${statusCode})${debugText ? `, ${debugText}` : ""}${fabricIndex !== void 0 ? `, fabricIndex: ${fabricIndex}` : ""}`
    );
    ...
    } else if (context === "updateFabricLabel" && statusCode === import_operational_credentials.OperationalCredentials.NodeOperationalCertStatus.LabelConflict) {
    ...
    throw new import_CommissioningError.CommissioningError(
      `Commission error for "${context}": ${import_operational_credentials.OperationalCredentials.NodeOperationalCertStatus[statusCode]} (${statusCode})${debugText ? `, ${debugText}` : ""}${fabricIndex !== void 0 ? `, fabricIndex: ${fabricIndex}` : ""}`
    );
  }
  /** Helper method to check for errorCode/debugTest responses and throw error on failure */
  ...
  #connectNetworkFailsafeTime(connectMaxTimeSeconds) {
    const pessimistic = this.interaction.channelType === import_general.ChannelType.BLE || this.collectedCommissioningData.supportsConcurrentConnection === false;
    if (pessimistic) {
      return import_general.Duration.max(
        (0, import_general.Millis)((0, import_general.Seconds)(connectMaxTimeSeconds) + CASE_RECONNECT_TIMEOUT),
        this.#defaultFailSafeTime
      );
    }
    return (0, import_general.Seconds)(connectMaxTimeSeconds);
  }
  /**
  ...
  #isConnectNetworkTransportError(error) {
    return (0, import_general.causedBy)(
      error,
      import_Ble.BleDisconnectedError,
      // also matches BleChannelClosedError subclass
      import_PeerCommunicationError.PeerUnresponsiveError,
  ...
  async #synchronizeTime() {
    if (this.collectedCommissioningData.rootServerList !== void 0 && this.collectedCommissioningData.rootServerList.find((clusterId) => clusterId === import_time_synchronization.TimeSynchronization.id)) {
  ...
  async #deviceAttestation() {
    ...
    let baseError;
    try {
      const result = await import_DeviceAttestationValidator.DeviceAttestationValidator.validate(
        {
      ...
      import_DeviceAttestationValidator.DeviceAttestationError.accept(error);
    ...
    if (findings.length > 0) {
      ...
      if (!decision.proceed) {
        baseError ??= new import_CommissioningError.CommissioningError(
          `Device attestation produced ${findings.length} finding(s) and was rejected by policy`
        );
        if (decision.reason !== void 0) {
          throw new import_CommissioningError.CommissioningError(decision.reason, { cause: baseError });
        }
        throw baseError;
      }
    ...
    this.#dacPublicKey = (0, import_general.PublicKey)(import_AttestationCertificates.Dac.fromAsn1(deviceAttestation).cert.ellipticCurvePublicKey);
    return {
      code: 0 /* Success */,
      breadcrumb: this.lastBreadcrumb
    };
  ...
  async #resolveAttestationFindings(findings) {
    ...
    if (this.#dacPublicKey !== void 0) {
      try {
        await this.fabric.crypto.verifyEcdsa(
          this.#dacPublicKey,
          import_general.Bytes.concat(nocsrElements, this.commissioningOptions.attestation.challengeKey),
          new import_general.EcdsaSignature(csrSignature)
        );
      } catch {
        throw new import_CommissioningError.CommissioningError("CSR signature verification failed against DAC public key");
      }
    }
  ...
  async #configureNetworkWifi() {
    ...
    if (this.collectedCommissioningData.supportsConcurrentConnection !== false) {
      ...
      if (networkingStatus !== import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus.Success) {
        wifiScanFailureHint = `scan failed: status=${import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus[networkingStatus]} (${networkingStatus})${debugText ? `, ${debugText}` : ""}`;
        logger.warn(
          `WiFi network scan for "${this.commissioningOptions.wifiNetwork.wifiSsid}" returned status ${import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus[networkingStatus]} (${networkingStatus})${debugText ? `: ${debugText}` : ""} - attempting connection anyway`
        );
      } else if (wiFiScanResults === void 0 || wiFiScanResults.length === 0) {
    ...
    if (addNetworkingStatus !== import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus.Success) {
      throw new WifiNetworkSetupFailedError(
        `Commissionee failed to add WiFi network "${this.commissioningOptions.wifiNetwork.wifiSsid}"${addDebugText ? `: ${addDebugText}` : ""}${wifiScanFailureHint !== void 0 ? ` (${wifiScanFailureHint} - verify network name and availability)` : ""}`
      );
    }
    ...
    if (connectResult.networkingStatus !== import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus.Success) {
      throw new WifiNetworkSetupFailedError(
        `Commissionee failed to connect to WiFi network "${this.commissioningOptions.wifiNetwork.wifiSsid}"${connectResult.debugText ? `: ${connectResult.debugText}` : ""}${wifiScanFailureHint !== void 0 ? ` (${wifiScanFailureHint} - verify network name and availability)` : ""}`
      );
    }
    this.collectedCommissioningData.successfullyConnectedToNetwork = true;
    logger.debug(
  ...
  async #configureNetworkThread() {
    ...
    if (!this.commissioningOptions.threadNetwork?.networkName) {
      ...
      if (networkingStatus2 !== import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus.Success) {
        threadScanFailureHint = `scan failed: status=${import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus[networkingStatus2]} (${networkingStatus2})${debugText2 ? `, ${debugText2}` : ""}`;
        logger.warn(
          `Thread network scan returned status ${import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus[networkingStatus2]} (${networkingStatus2})${debugText2 ? `: ${debugText2}` : ""} - attempting connection anyway`
        );
      } else if (threadScanResults === void 0 || threadScanResults.length === 0) {
        ...
        if (wantedNetworkFound === void 0) {
          ...
          logger.warn(
            `Thread network "${this.commissioningOptions.threadNetwork.networkName}" not found in scan results: ${import_general.Diagnostic.json(threadScanResults)} - attempting connection anyway`
          );
        } else {
          logger.debug(
    ...
    if (addNetworkingStatus !== import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus.Success) {
      throw new ThreadNetworkSetupFailedError(
        `Commissionee failed to add Thread network: status=${import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus[addNetworkingStatus]} (${addNetworkingStatus})${addDebugText ? `, ${addDebugText}` : ""}${threadScanFailureHint !== void 0 ? ` (${threadScanFailureHint} - verify network name and availability)` : ""}`
      );
    }
    if (networkIndex === void 0) {
    ...
    if (networkingStatus !== import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus.Success) {
      throw new ThreadNetworkSetupFailedError(
        `Commissionee failed to connect to Thread network: status=${import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus[networkingStatus]} (${networkingStatus})${debugText ? `, ${debugText}` : ""}${threadScanFailureHint !== void 0 ? ` (${threadScanFailureHint} - verify network name and availability)` : ""}`
      );
    }
    logger.debug(
