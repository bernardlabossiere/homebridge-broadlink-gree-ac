...
class PeerSet {
  ...
  async #openSocket(address, abort) {
    const channelType = "type" in address && address.type === "tcp" ? import_general.ChannelType.TCP : import_general.ChannelType.UDP;
    let lookupAddress;
    if (channelType === import_general.ChannelType.UDP) {
      lookupAddress = (0, import_general.isIPv6)(address.ip) ? "::" : "0.0.0.0";
    }
    ...
    if (operationalInterface === void 0) {
      throw new import_general.NetworkUnreachableError(
        `No ${channelType.toUpperCase()} interface available for address ${address.ip}`
      );
    }
    if (abort.aborted) return void 0;
    try {
      const result = await import_general.Abort.race(abort, operationalInterface.openChannel(address, { abort }));
...
function operationalAddressOf(session) {
  ...
  if (session.channel.transportChannel.type === import_general.ChannelType.TCP) {
    return;
  }
