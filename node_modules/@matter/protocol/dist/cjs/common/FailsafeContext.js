 ...
 */
const logger = import_general.Logger.get("FailsafeContext");
class MatterFabricInvalidAdminSubjectError extends import_general.MatterFlowError {
}
class FailsafeContext {
  ...
  constructor(options) {
    ...
    this.#construction = (0, import_general.Construction)(this, async () => {
      this.#fabricBuilder = await import_Fabric.FabricBuilder.create(this.#fabrics.crypto);
      ...
      logger.debug(`Arm failSafe timer for ${import_general.Duration.format(expiryLength)}`);
      const closedByPeer = () => {
  ...
  async extend(fabric, expiryLength, currentExchange) {
    ...
    if (expiryLength > 0) {
      logger.debug(`Extend failSafe timer for ${import_general.Duration.format(expiryLength)}`);
    }
  }
  get fabricIndex() {
  ...
  async completeCommission() {
    if (this.#failsafe === void 0) {
      throw new import_general.MatterFlowError("armFailSafe should be called first!");
    }
    ...
    await this.closePaseSession({ cause: new import_PeerCommunicationError.FailsafeExpiredError() });
    await this.close();
  }
  getFailSafeContext() {
    if (this.#failsafe === void 0) throw new import_general.MatterFlowError("armFailSafe should be called first!");
  ...
  createCertificateSigningRequest(isForUpdateNoc, sessionId) {
    if (this.#fabrics.findByKeypair(this.#builder.keyPair)) {
      throw new import_general.MatterFlowError("Key pair already exists.");
    }
  ...
  async closePaseSession(context) {
  ...
  async close(currentExchange) {
    await this.#construction.close(async () => {
      ...
      this.#detachClosedByPeer = void 0;
      if (this.#failsafe) {
  ...
  async buildUpdatedFabric(nocValue, icacValue) {
    if (this.associatedFabric === void 0) {
      throw new import_general.MatterFlowError("No fabric associated with failsafe context, but we prepare an Fabric update.");
    }
  ...
  async buildFabric(nocData) {
    ...
    if (!import_types.NodeId.isOperationalNodeId(caseAdminSubject) && !import_types.NodeId.isCaseAuthenticatedTag(caseAdminSubject)) {
      try {
        if (import_types.CaseAuthenticatedTag.getVersion(import_types.NodeId.extractAsCaseAuthenticatedTag(caseAdminSubject)) === 0) {
          throw new MatterFabricInvalidAdminSubjectError();
        }
      } catch (error) {
        if (error instanceof import_types.ValidationError || error instanceof import_general.UnexpectedDataError) {
          throw new MatterFabricInvalidAdminSubjectError();
        } else {
          throw error;
        }
  ...
  async rollback(currentExchange) {
    ...
    await this.closePaseSession({ cause: new import_PeerCommunicationError.FailsafeExpiredError(), currentExchange });
    let fabric = void 0;
    if (this.fabricIndex !== void 0) {
      ...
      if (this.#fabrics.has(fabricIndex)) {
        ...
        for (const session of this.#sessions.sessionsForFabricIndex(fabricIndex)) {
          await session.initiateForceClose({ cause: new import_PeerCommunicationError.FabricChangedError(), currentExchange });
        }
      }
    }
  ...
  get #builder() {
    if (this.#fabricBuilder === void 0) {
      throw new import_general.UninitializedDependencyError("FailsafeContext", "Fabric builder has not been initialized");
    }
