...
class MessageExchange {
  ...
  #receivedMessageAckTimer = import_general.Time.getTimer("ack receipt timeout", import_MRP.MRP.STANDALONE_ACK_TIMEOUT, () => {
    if (this.#receivedMessageToAck !== void 0) {
      ...
      this.sendStandaloneAckForMessage(messageToAck).catch(
        (error) => logger.warn("An error happened when sending a standalone ack", error)
      );
    }
  });
  #closeTimer;
  ...
  constructor(config) {
    ...
    logger.debug(
      ...
      import_general.Diagnostic.dict({
        ...
        address: addressOverride ? import_general.ServerAddress.urlFor(addressOverride) : void 0,
  ...
  async onMessageReceived(message, duplicate = false) {
    ...
    const isStatusReport = import_types.SecureMessageType.isStatusReport(protocolId, messageType);
  ...
  async send(messageType, payload, options = {}) {
    ...
    try {
      ...
      if (!options.suppressPeerLoss && this.#messageReceivedCounter === 0 && (0, import_general.causedBy)(e, import_PeerCommunicationError.TransientPeerCommunicationError, import_general.TimeoutError, import_general.NetworkError)) {
  ...
  async #send(messageType, payload, standaloneAckMessageId) {
    ...
    try {
      ...
      const isStatusReport = messageType === import_types.SecureMessageType.StatusReport;
      const protocolId = isStandaloneAck || isStatusReport ? import_types.SECURE_CHANNEL_PROTOCOL_ID : this.#protocolId;
      if (isStandaloneAck) {
        if (!this.session.usesMrp) {
          return;
        }
  ...
  async nextMessage(options) {
    try {
      ...
      if (!options?.suppressPeerLoss && this.#messageReceivedCounter === 0 && (0, import_general.causedBy)(e, import_PeerCommunicationError.TransientPeerCommunicationError)) {
        await this.#context.peerLost(this, (0, import_general.asError)(e));
      }
      throw e;
    }
  ...
  #retransmitMessage(message, expectedProcessingTime) {
    ...
    this.channel.send(message, {
      ...
      logger.warn(`Error retransmitting ${import_MessageCodec.Message.via(this, message)}:`, error);
      if (error instanceof import_errors.SessionClosedError) {
        this.#close().catch((error2) => logger.warn("An error happened when closing the exchange", error2));
      } else {
  ...
  startTimedInteraction(timeout) {
    if (this.#timedInteractionTimer !== void 0 && this.#timedInteractionTimer.isRunning) {
      ...
      throw new import_types.StatusResponseError("Timed interaction already running for this exchange", import_types.Status.InvalidAction);
    }
    logger.debug(
