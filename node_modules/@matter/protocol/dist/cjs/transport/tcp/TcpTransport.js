...
class TcpTransport {
  ...
  async openChannel(address, options) {
  ...
  async #connect(address, abort) {
    ...
    if (abort?.aborted) {
      await channel.close();
      throw new import_general.NetworkError("TCP connect aborted after socket established");
    }
