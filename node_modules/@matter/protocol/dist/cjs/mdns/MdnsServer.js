...
class MdnsServer {
  ...
  #suppressedByKnownAnswer(record, knownAnswer) {
    const lcName = knownAnswer.name.toLowerCase();
    if (record.name.toLowerCase() !== lcName) return false;
    return (0, import_general.isDeepEqual)({ ...record, name: lcName }, { ...knownAnswer, name: lcName }, true);
  }
  ...
  #shouldSuppressResponse(queries, knownAnswers, sourceIntf, answers) {
    ...
    const queryKey = queries.map((q) => `${q.name.toLowerCase()}-${q.recordType}`).sort().join("|") + `-${sourceIntf}`;
