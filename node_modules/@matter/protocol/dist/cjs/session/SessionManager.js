...
class ShutdownError extends import_general.ClosedError {
  constructor(message = "Local node shutdown", options) {
...
class SessionManager {
  ...
  createUnsecuredSession(options) {
    ...
    while (true) {
      ...
      if (this.#unsecuredSessions.has(ephemeralNodeId)) {
        if (++tries > 4) {
          throw new import_general.InternalError("Unable to allocate unique ephemeral node ID; entropy source is broken");
        }
        continue;
      }
  ...
  handlePeerShutdown(address, asOf) {
    return this.#handlePeerLoss({
      address,
      cause: new import_PeerCommunicationError.PeerShutdownError(),
      asOf: asOf ?? import_general.Time.nowMs,
   ...
   */
  async handlePeerLoss(address, cause, asOf) {
    return await this.#handlePeerLoss({ address, asOf: asOf ?? import_general.Time.nowMs, cause });
  }
  ...
  groupSessionFromPacket(packet, aad) {
    ...
    if (groupId === void 0) {
      throw new import_general.UnexpectedDataError("Group ID is required for GroupSession fromPacket.");
    }
  ...
  async closeAllSessions() {
    ...
    const context = { cause: new ShutdownError("Session closed by node shutdown") };
    ...
    await import_general.MatterAggregateError.allSettled(closePromises, "Error closing sessions").catch(
      (error) => logger.warn("Error closing sessions:", error)
    );
  }
  /**
