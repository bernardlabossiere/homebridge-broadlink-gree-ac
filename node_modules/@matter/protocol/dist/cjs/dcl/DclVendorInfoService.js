...
class DclVendorInfoService {
  ...
  async #consumeVendorSeed(source) {
    ...
    try {
      for await (const entry of source.entries) {
        ...
        try {
          if (typeof entry.vendorId !== "number" || typeof entry.vendorName !== "string" || !entry.vendorName || entry.kind !== "production" && entry.kind !== "test") {
            throw new import_DclClient.MatterDclError("invalid vendor entry shape");
          }
          collected.push({
          ...
          logger.warn("seed: malformed vendor entry, aborting stream", import_general.Diagnostic.errorMessage((0, import_general.asError)(err)));
          break;
        }
      }
    } catch (err) {
      logger.warn("seed: vendor stream failed", import_general.Diagnostic.errorMessage((0, import_general.asError)(err)));
      return;
    }
