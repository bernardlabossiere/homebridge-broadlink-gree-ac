...
class DclCertificateService {
  ...
  constructor(environment, options = {}) {
    ...
    logger.info(
      ...
      import_general.Diagnostic.dict({
        ...
        flags: import_general.Diagnostic.asFlags({ acceptTest: this.acceptsTestCertificates })
      })
    );
    this.#revocationCache = new import_general.AsyncCache(
      "CRL revocation",
      (akid) => this.#fetchAndValidateRevocation(akid),
      (0, import_general.Hours)(1)
    );
   ...
   */
  getCertificate(subjectKeyId, options) {
    this.construction.assert();
    const metadata = this.#certificateIndex.get(this.#normalizeSubjectKeyId(subjectKeyId));
    if (metadata === void 0 || !this.#isRelevant(metadata, options)) {
      return void 0;
    }
    return metadata;
  }
   ...
   */
  async getCertificateAsPem(subjectKeyId, options) {
    this.construction.assert();
  ...
  async #getCertificateDer(subjectKeyId, options) {
    ...
    return import_general.Bytes.of(derBytes);
  }
  /**
  ...
  async isRevoked(authorityKeyIdentifier, serialNumber, issuerDnDerHex) {
    ...
    const serialHex = typeof serialNumber === "string" ? serialNumber.replace(/:/g, "").toUpperCase() : import_general.Bytes.toHex(serialNumber).toUpperCase();
  ...
  async getOrFetchCertificate(subjectKeyId, options) {
    ...
    if (this.#fetchPromise !== void 0) {
      ...
      return afterFetch && this.#isRelevant(afterFetch, options) ? afterFetch : void 0;
    }
    try {
      ...
      return fetched && this.#isRelevant(fetched, options) ? fetched : void 0;
    } catch (error) {
  ...
  async addCertificate(derBytes, kind, options) {
    this.construction.assert();
    const cert = import_Certificate.Certificate.parseAsn1Certificate(
      derBytes,
      kind === "PAA" ? import_Certificate.Certificate.REQUIRED_PAA_EXTENSIONS : import_Certificate.Certificate.REQUIRED_EXTENSIONS
    );
    ...
    await this.#storeCertificate(this.#storage, skid, import_general.Bytes.of(derBytes), {
      subject: cert.subject.commonName,
      subjectKeyId: skid,
      serialNumber: import_general.Bytes.toHex(cert.serialNumber),
      vid,
      isRoot: kind === "PAA",
      isProduction,
      kind
    ...
    await this.#saveIndex();
    logger.info(`Added ${kind} certificate`, import_general.Diagnostic.dict({ skid, isProduction }));
    return true;
  }
  /**
  ...
  async getOrFetchCdSigner(subjectKeyId) {
    ...
    if (existing !== void 0 && (existing.kind ?? "PAA") === "CDSigner") {
      ...
      const cert = import_Certificate.Certificate.parseAsn1Certificate(derBytes, import_Certificate.Certificate.REQUIRED_EXTENSIONS);
      return { publicKey: cert.ellipticCurvePublicKey, isProduction: existing.isProduction };
    }
    try {
      const config = this.#options.dclConfig ?? import_DclConfig.DclConfig.production;
      const dclClient = new import_DclClient.DclClient(config);
      const certs = await dclClient.fetchCertificatesBySubjectKeyId(normalizedSkid, this.#options);
      if (certs.length === 0) {
        return void 0;
      }
      const derBytes = import_general.Pem.asDer(certs[0].pemCert);
  ...
  async update(force = false) {
    ...
    try {
      ...
      logger.info("Certificate update failed", import_general.Diagnostic.errorMessage((0, import_general.asError)(error)));
      return false;
    }
    return true;
  }
  ...
  async #fetchGitHubCertificates(storage, force) {
    try {
      ...
      logger.info("Failed to fetch certificates from GitHub", import_general.Diagnostic.errorMessage((0, import_general.asError)(error)));
    }
  }
  /**
  ...
  async #fetchGitHubCdSignerCertificates(storage, force) {
    try {
      ...
      const { owner, repo, branch, cdSignerCertPath } = this.#options.githubConfig ?? import_DclConfig.DclGithubConfig.defaults;
      const repoClient = new import_general.Github.Repo(owner, repo, branch, this.#options);
      ...
      logger.info("Failed to fetch CD signer certificates from GitHub", import_general.Diagnostic.errorMessage((0, import_general.asError)(error)));
    }
  }
  /**
  ...
  async #fetchGitHubCdSignerCertificate(storage, certDir, filename, force) {
    try {
      ...
      if (!force && this.#certificateIndex.has(subjectKeyId)) {
        logger.debug(`CD signer certificate already exists, skipping`, import_general.Diagnostic.dict({ skid: subjectKeyId }));
        return;
      }
      const subject = filename.replace(".der", "");
      const serialNumber = import_general.Bytes.toHex(cert.serialNumber);
      const vid = cert.subject.vendorId ?? 0;
      await this.#storeCertificate(storage, subjectKeyId, import_general.Bytes.of(derBytes), {
        subject,
        subjectKeyId,
        serialNumber,
        vid,
      ...
      logger.debug(`Stored GitHub CD signer certificate`, import_general.Diagnostic.dict({ skid: subjectKeyId, filename }));
    } catch (error) {
      logger.info(
  ...
  async #fetchGitHubCertificate(storage, certDir, filename, force) {
    try {
      ...
      logger.info(`Failed to fetch GitHub certificate ${filename}`, import_general.Diagnostic.errorMessage((0, import_general.asError)(error)));
    }
  }
  /**
  ...
  async #storeCertificate(storage, subjectKeyId, derBytes, metadata) {
    ...
    if (metadata.fetchedAt === void 0) {
      metadata = { ...metadata, fetchedAt: existing?.fetchedAt ?? import_general.Time.nowMs };
    }
    await storage.set(subjectKeyId, derBytes);
    this.#certificateIndex.set(subjectKeyId, metadata);
  }
  async #consumeCertSeed(source, certKind) {
    const inserted = new Array();
    let expiredCount = 0;
    ...
    try {
      for await (const entry of source.entries) {
        ...
        try {
          if (typeof entry.subjectKeyId !== "string" || !entry.subjectKeyId || typeof entry.derHex !== "string" || !entry.derHex || entry.kind !== "production" && entry.kind !== "test") {
            throw new import_DclClient.MatterDclError("invalid entry shape");
          }
          if (entry.kind === "test" && !this.#options.fetchTestCertificates) {
            continue;
          }
          if (entry.notAfter !== void 0 && Date.parse(entry.notAfter) < Date.now()) {
            expiredCount++;
            continue;
          }
          const der = import_general.Bytes.fromHex(entry.derHex);
          const parsed = import_Certificate.Certificate.parseAsn1Certificate(
            der,
            isPaa ? import_Certificate.Certificate.REQUIRED_PAA_EXTENSIONS : import_Certificate.Certificate.REQUIRED_EXTENSIONS
          );
          ...
          if (skidFromDer !== entrySkid) {
            logger.warn(
              ...
              import_general.Diagnostic.dict({ certKind })
            );
            continue;
          }
          ...
          const vid = parsed.subject.vendorId ?? 0;
          await this.#storeCertificate(this.#storage, skidFromDer, import_general.Bytes.of(der), {
            subject: parsed.subject.commonName,
            subjectKeyId: skidFromDer,
            serialNumber: import_general.Bytes.toHex(parsed.serialNumber),
            vid,
            isRoot: isPaa,
            isProduction,
            kind: certKind,
          ...
          logger.warn("seed: malformed entry, aborting stream", import_general.Diagnostic.errorMessage((0, import_general.asError)(err)));
          break;
        }
      }
    } catch (err) {
      logger.warn("seed: stream failed", import_general.Diagnostic.errorMessage((0, import_general.asError)(err)));
    }
  ...
  async #fetchAndValidateRevocation(akid) {
    ...
    const revocationTimeout = (0, import_general.Seconds)(3);
    const config = this.#options.dclConfig ?? import_DclConfig.DclConfig.production;
    const dclClient = new import_DclClient.DclClient(config);
    let points;
    try {
      ...
      if (error instanceof import_DclClient.MatterDclResponseError && error.response.code === 5) {
    ...
    const crlPoints = points.filter((p) => p.revocationType === import_types.RevocationTypeEnum.Crl);
    ...
    throw new import_DclClient.MatterDclError(`All CRL downloads failed for AKID ${akid}`);
  }
  /**
  ...
  async #processRevocationPoint(point, timeout) {
    ...
    if (!response.ok) {
      throw new import_DclClient.MatterDclError(`Failed to fetch CRL from ${point.dataUrl}: ${response.status}`);
    }
    ...
    if (point.dataFileSize !== void 0) {
      ...
      if (expectedSize > 0 && crlBytes.length !== expectedSize) {
        throw new import_DclClient.MatterDclError(`CRL size mismatch: expected ${expectedSize} bytes, got ${crlBytes.length}`);
      }
    }
    if (point.dataDigest !== void 0 && point.dataDigestType !== void 0) {
      const algorithm = import_general.HashFipsAlgorithmId[point.dataDigestType];
      if (algorithm !== void 0) {
        const actualDigest = import_general.Bytes.toBase64(await this.#crypto.computeHash(crlBytes, algorithm));
        if (actualDigest !== point.dataDigest) {
          throw new import_DclClient.MatterDclError(`CRL digest mismatch: expected ${point.dataDigest}, got ${actualDigest}`);
        }
      } else {
    ...
    if (signerPublicKey !== void 0 && crlParsed.tbsDer !== void 0 && crlParsed.signatureValue !== void 0 && import_general.Bytes.of(crlParsed.signatureValue).length > 0) {
      try {
        await this.#crypto.verifyEcdsa(
          (0, import_general.PublicKey)(signerPublicKey),
          crlParsed.tbsDer,
          new import_general.EcdsaSignature(crlParsed.signatureValue, "der")
        );
      } catch {
        throw new import_DclClient.MatterDclError("CRL signature verification failed against CRLSignerCertificate");
      }
    }
    return {
  ...
  async #validateCrlSigner(point) {
    const signerDer = import_general.Pem.asDer(point.crlSignerCertificate);
    const signerCert = point.isPAA ? import_AttestationCertificates.Paa.fromAsn1(signerDer) : import_AttestationCertificates.Pai.fromAsn1(signerDer);
    let delegatorCert;
    if (point.crlSignerDelegator) {
      const delegatorDer = import_general.Pem.asDer(point.crlSignerDelegator);
      delegatorCert = import_AttestationCertificates.Pai.fromAsn1(delegatorDer);
    }
    if (point.isPAA) {
      const signerVid = signerCert.cert.subject.vendorId;
      if (signerVid !== void 0 && signerVid !== point.vid) {
        throw new import_DclClient.MatterDclError(
      ...
      if (checkVid !== void 0 && checkVid !== point.vid) {
        throw new import_DclClient.MatterDclError(`CRL signer VendorID ${checkVid} does not match entry VendorID ${point.vid}`);
      }
      if (point.pid !== void 0) {
        const checkPid = certToCheck instanceof import_AttestationCertificates.Pai ? certToCheck.cert.subject.productId : void 0;
    ...
    if (signerAkid !== void 0) {
      ...
      const trustAllPaas = { considerTestCertificates: true };
      let issuerPublicKey;
      if (delegatorCert !== void 0) {
        ...
        if (delegatorAkid === void 0 || !this.#certificateIndex.has(this.#normalizeSubjectKeyId(delegatorAkid))) {
          throw new import_DclClient.MatterDclError("CRLSignerDelegator chain cannot be anchored to trusted PAA");
        }
        const paaDer = await this.#getCertificateDer(delegatorAkid, trustAllPaas);
        const paa = import_AttestationCertificates.Paa.fromAsn1(paaDer);
        await this.#crypto.verifyEcdsa(
          (0, import_general.PublicKey)(paa.cert.ellipticCurvePublicKey),
      ...
      await this.#crypto.verifyEcdsa(
        (0, import_general.PublicKey)(issuerPublicKey),
  ...
  static parseCrl(crlDer) {
    ...
    const decoded = import_general.DerCodec.decode(crlDer);
    ...
    const tbsDer = import_general.DerCodec.encode(tbsCertList);
    let signatureValue;
    if (certListElements.length >= 3 && certListElements[2]._tag === 3) {
      signatureValue = import_general.Bytes.of(certListElements[2]._bytes);
    }
    let idx = 0;
    if (tbsElements[idx]?._tag === import_general.DerType.Integer) {
      idx++;
    }
    if (tbsElements[idx]?._tag === import_general.DerTag.Sequence) {
      idx++;
    }
    let issuerDnDerHex;
    if (tbsElements[idx]?._tag === import_general.DerTag.Sequence) {
      const issuerNode = tbsElements[idx];
      issuerDnDerHex = import_general.Bytes.toHex(import_general.DerCodec.encode(issuerNode)).toUpperCase();
      idx++;
    }
    if (tbsElements[idx]?._tag === import_general.DerType.UtcDate || tbsElements[idx]?._tag === import_general.DerType.GeneralizedTime) {
      idx++;
    }
    let nextUpdateMs;
    if (tbsElements[idx]?._tag === import_general.DerType.UtcDate || tbsElements[idx]?._tag === import_general.DerType.GeneralizedTime) {
      try {
        const dateStr = import_general.Bytes.toString(tbsElements[idx]._bytes);
        let year;
        let rest;
        if (tbsElements[idx]._tag === import_general.DerType.UtcDate) {
    ...
    for (const element of tbsElements) {
      if (element._tag === 160 && element._bytes) {
        const extSequence = import_general.DerCodec.decode(element._bytes);
        if (extSequence._elements) {
          for (const ext of extSequence._elements) {
            if (!ext._elements || ext._elements.length < 2) continue;
            const oid = import_general.Bytes.toHex(ext._elements[0]._bytes);
            if (oid === "551d23") {
              const valueIdx = ext._elements.length > 2 ? 2 : 1;
              const akiOctetString = ext._elements[valueIdx];
              if (akiOctetString._bytes) {
                const akiSeq = import_general.DerCodec.decode(akiOctetString._bytes);
                if (akiSeq._elements) {
                  for (const akiField of akiSeq._elements) {
                    if (akiField._tag === 128) {
                      authorityKeyId = import_general.Bytes.toHex(import_general.Bytes.of(akiField._bytes)).toUpperCase();
                      break;
                    }
                  }
                }
    ...
    for (const element of tbsElements) {
      if (element._tag === import_general.DerTag.Sequence && element._elements) {
        const firstChild = element._elements[0];
        if (firstChild && firstChild._tag === import_general.DerTag.Sequence && firstChild._elements && firstChild._elements[0]?._tag === import_general.DerType.Integer) {
    ...
    for (const entry of revokedCertsNode._elements) {
      if (entry._tag !== import_general.DerTag.Sequence || !entry._elements || entry._elements.length < 1) continue;
      const serialNode = entry._elements[0];
      if (serialNode._tag !== import_general.DerType.Integer) continue;
      const serialHex = import_general.Bytes.toHex(import_general.Bytes.of(serialNode._bytes)).toUpperCase();
