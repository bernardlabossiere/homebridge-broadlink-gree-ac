...
function Invoke(optionsOrData, ...commands) {
  ...
  const {
    interactionModelRevision = import_SessionParameters.SessionParameters.fallbacks.interactionModelRevision,
  ...
  return {
    ...
    [import_general.Diagnostic.value]: () => import_general.Diagnostic.list(
      commands.map((cmd) => {
        ...
        return [
          import_general.Diagnostic.strong(Invoke.isLegacy(cmd) ? "(legacy)" : (0, import_Specifier.resolvePathForSpecifier)(cmd)),
          "with",
...
((Invoke2) => {
  function Command(request, skipValidation = false) {
    ...
    if (typeof request.command !== "string" && "requestId" in request.command) {
      const legacy = request.command;
      requestSchema = legacy.requestSchema;
      requestId = legacy.requestId;
      timed = legacy.timed;
    } else {
      const command = Invoke2.commandOf(request);
      requestSchema = (0, import_types.TlvOfModel)(command.schema) ?? import_types.TlvVoid;
    ...
    if (requestSchema instanceof import_types.ObjectSchema && fields === void 0) {
      fields = {};
    }
    if (!skipValidation) {
