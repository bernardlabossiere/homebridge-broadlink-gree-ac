...
const GlobalAttrIds = import_model.AttributeModel.globalIds;
class AttributeReadResponse extends import_DataResponse.DataResponse {
  ...
  *readEndpointForWildcard(endpoint, path) {
  ...
  addStatus(path, status) {
    logger.debug(
      () => `Error reading attribute ${this.node.inspectPath(path)}: Status=${import_types.Status[status]}(${status})`
    );
    const report = {
      kind: "attr-status",
      path,
