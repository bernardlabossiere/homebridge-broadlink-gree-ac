...
class AttributeWriteResponse extends import_DataResponse.DataResponse {
      ...
      for (const { path, data, dataVersion } of writeRequests) {
        ...
        if (path.endpointId === void 0 || path.clusterId === void 0 || path.attributeId === void 0) {
          ...
          if (import_Subject.Subject.isGroup(this.session.subject)) {
            throw new import_types.StatusResponseError("Group writes can not be concrete paths", import_types.Status.InvalidAction);
          }
          writeResponses.push(
  ...
  async #processWildcard(path, value) {
    ...
    if (isGroupPath) {
      throw new import_types.StatusResponseError("Illegal write request with group ID and endpoint ID", import_types.Status.InvalidAction);
    }
  ...
  #writeClusterForWildcard(cluster, path, value) {
    ...
    if (attributeId === void 0) {
      throw new import_types.StatusResponseError("Wildcard path write must specify an attributeId", import_types.Status.InvalidAction);
    } else {
      const attribute = cluster.type.attributes[attributeId];
      if (attribute !== void 0) {
  ...
  #asStatus(path, status, clusterStatus) {
    if (status !== import_types.Status.Success) {
      logger.debug(
        () => `Error writing attribute ${this.node.inspectPath(path)}: Status=${import_types.Status[status]}(${(0, import_general.toHex)(status)}), ClusterStatus=${clusterStatus !== void 0 ? (0, import_general.toHex)(clusterStatus) : void 0}`
      );
    }
    const report = {
      kind: "attr-status",
