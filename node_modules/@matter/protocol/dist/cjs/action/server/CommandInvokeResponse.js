...
class CommandInvokeResponse extends import_DataResponse.DataResponse {
      ...
      for (const command of invokeRequests) {
        ...
        if (path.endpointId === void 0 || path.clusterId === void 0 || path.commandId === void 0) {
          ...
          if (import_Subject.Subject.isGroup(this.session.subject)) {
            throw new import_types.StatusResponseError("Group commands cannot be concrete paths", import_types.Status.InvalidAction);
          }
  ...
  #processWildcard(path, commandRef, commandFields) {
    ...
    if (isGroupPath && endpointId !== void 0) {
      throw new import_types.StatusResponseError("Illegal command invoke with group ID and endpoint ID", import_types.Status.InvalidAction);
    }
  ...
  #addStatus(path, commandRef, status, clusterStatus) {
    if (status !== import_types.Status.Success) {
      logger.info(
        () => `Invoke error ${this.node.inspectPath(path)}: Status=${import_types.Status[status]}(${status}), ClusterStatus=${clusterStatus}`
      );
    }
    const response = {
      kind: "cmd-status",
  ...
  async #invokeCommand(command, path, commandRef, commandFields, invoker, skipValidation) {
    try {
      ...
      if (encodedResponse.length === 0) {
        this.#addStatus(path, commandRef, import_types.Status.Success);
      } else {
        this.#addResponse({
          kind: "cmd-response",
          path: {
      ...
      if (sre) {
        ...
        const errorLogText = `Error ${import_general.Diagnostic.hex(errorCode)}${sre.clusterCode !== void 0 ? `/${import_general.Diagnostic.hex(sre.clusterCode)}` : ""} while invoking command: ${sre.message}`;
        if (sre instanceof import_types.ValidationError) {
          logger.info(
            `Validation-${errorLogText}${sre.fieldName !== void 0 ? ` in field ${sre.fieldName}` : ""}`
          );
          if (errorCode === import_types.Status.InvalidAction) {
            errorCode = import_types.Status.InvalidCommand;
          }
        } else {
