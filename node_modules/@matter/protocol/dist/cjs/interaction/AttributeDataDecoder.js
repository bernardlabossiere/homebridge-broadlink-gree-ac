...
function normalizeAttributeStatus(data) {
  ...
  data.forEach((entry) => {
    ...
    const clusterModel = import_model.Matter.clusters(clusterId);
    const attributeModel = clusterModel?.attributes(attributeId) ?? import_model.Matter.attributes(attributeId);
    if (attributeModel === void 0) {
      result.push({
        path: {
          nodeId,
          endpointId,
...
function normalizeAndDecodeAttributeData(data) {
  ...
  responseList.forEach((values) => {
    ...
    try {
      ...
      const attributeModel = clusterModel?.attributes(attributeId) ?? import_model.Matter.attributes(attributeId);
      if (attributeModel === void 0) {
        const attributeName2 = `Unknown (${import_general.Diagnostic.hex(attributeId)})`;
        const value2 = decodeUnknownAttributeValue(values);
        result.push({
          path: { nodeId, endpointId, clusterId, attributeId, attributeName: attributeName2 },
          version: dataVersion,
          value: value2
        });
        return;
      ...
      let schema;
      try {
        schema = (0, import_types.TlvOfModel)(attributeModel);
      } catch {
        const value2 = decodeUnknownAttributeValue(values);
        result.push({
...
function decodeAttributeValueWithSchema(schema, values, defaultValue) {
  ...
  if (schema === import_types.TlvAny) {
    return decodeUnknownAttributeValue(values);
  }
  if (schema instanceof import_types.ArraySchema) {
