...
class InteractionMessenger {
  ...
  trackEndMessage(promise, errorMessage) {
    if (this.#endMessagePromise !== void 0) {
      throw new import_general.InternalError("An interaction can only have one end message");
    }
    this.#endMessagePromise = promise.catch((error) => logger.info(this.exchange.via, errorMessage, import_general.Diagnostic.errorMessage(error))).finally(() => this.#endMessagePromise = void 0);
  }
...
class InteractionServerMessenger extends InteractionMessenger {
  async handleRequest(recipient) {
    ...
    try {
      ...
      if ((0, import_general.causedBy)(error, import_general.NoResponseTimeoutError, import_PeerCommunicationError.PeerUnresponsiveError)) {
      ...
      if (sre) {
        logger.info(
          ...
          import_general.Diagnostic.strong(`${import_types.Status[sre.code]}#${sre.code}`),
          "due to error:",
          import_general.Diagnostic.errorMessage(sre)
        );
  ...
  async sendDataReport(options) {
    ...
    const dataReport = {
      subscriptionId,
      suppressResponse,
      interactionModelRevision: interactionModelRevision ?? import_model.Specification.INTERACTION_MODEL_REVISION,
...
class IncomingInteractionClientMessenger extends InteractionMessenger {
  async waitFor(expectedMessageInfo, messageType, options) {
   ...
   */
  async *readDataReports(options) {
    while (true) {
      const dataReportMessage = await this.waitFor("DataReport", 5 /* ReportData */, options);
...
class InteractionClientMessenger extends IncomingInteractionClientMessenger {
  static async create(exchangeProvider, options) {
    const exchange = await exchangeProvider.initiateExchange(options);
    return new this(exchange);
  }
  async sendReadRequest(readRequest, options) {
    await this.send(2 /* ReadRequest */, this.#encodeReadingRequest(import_types.TlvReadRequest, readRequest), options);
  }
  ...
  async sendSubscribeRequest(subscribeRequest, options) {
    const request = this.#encodeReadingRequest(import_types.TlvSubscribeRequest, subscribeRequest);
    await this.send(3 /* SubscribeRequest */, request, options);
  }
  /**
   ...
   */
  async sendInvokeCommand(invokeRequest, options) {
    ...
    await this.send(8 /* InvokeRequest */, import_types.TlvInvokeRequest.encode(invokeRequest), options);
    const allInvokeResponses = [];
    let finalResponse;
    while (true) {
      const responseMessage = await this.nextMessage(9 /* InvokeResponse */, options, "InvokeResponse");
  ...
  async sendWriteCommand(writeRequest, options) {
    if (writeRequest.suppressResponse) {
      await this.requestWithSuppressedResponse(6 /* WriteRequest */, import_types.TlvWriteRequest, writeRequest, options);
    } else {
  ...
  sendTimedRequest(timeout, options) {
    return this.request(
      10 /* TimedRequest */,
      import_types.TlvTimedRequest,
      1 /* StatusResponse */,
      import_types.TlvStatusResponse,
      {
        timeout,
  ...
  async requestWithSuppressedResponse(requestMessageType, requestSchema, request, options) {
    try {
  ...
  async request(requestMessageType, requestSchema, responseMessageType, responseSchema, request, options) {
    await this.send(requestMessageType, requestSchema.encode(request), {
      expectAckOnly: false,
      expectedProcessingTime: options?.expectedProcessingTime,
      abort: options?.abort
